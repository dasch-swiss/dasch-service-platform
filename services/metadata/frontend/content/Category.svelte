<script lang="ts">
  ...
  let categories = [
    ...
    { id: 2, isOpen: false, name: 'Type of data', sub: [] },
    ...
    { id: 8, isOpen: false, name: 'Organization', sub: [] },
  ];

  const toggleButton = (cat: any) => (event: any) => {
    let bool = cat.isOpen;
...
{#each categories as category }
  <button on:click={toggleButton(category)}>{category.name}</button>
  {#if category.sub && category.sub.length > 0}
    <div class="{category.isOpen ? 'visible' : 'in-visible'}">
      {#each category.sub as sub}
        <div><input type="radio" />{sub}</div>
      {/each}
    </div>
  {/if}
{/each}
